...
export class Player {
	...
	addExpandingCircle() {
		let circle = this.pad.game.add.sprite( this.sprite.x ,	this.sprite.y );

		let timer = this.pad.game.time.create(false);
		timer.repeat(30, 50, () => {circle.scale.multiply(1.1, 1.1);} );
		timer.start();
